...
export class DropPlacesComponent {
	...
	onDrop(event: any) {
		if (!event.previousContainer.id.startsWith(event.container.id.substring(0, event.container.id.length - 1))) {
			...
			this.selectedCards[dropIndex].disabled;
		}
		...
		//console.log(event.container.data);
		/*copyArrayItem(this.playerCards,
			[this.selectedCards],
			cardIndex,
			Number(event.container.id.match(/(\d)/g)));*/
		//console.log(this.selectedCards[dropIndex]);

		//console.log(this.selectedCards);
		//eval(` = ${this.playerCards[cardIndex]}`);
		//console.log(selectedCard);
		//selectedCard = this.selectedCards[cardIndex];
		//this.selectedCard1 = selectedCard;
